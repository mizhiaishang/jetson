...
class AirSimExplorer:
    ...
    def initialize_airsim_client(self):#以完成修改（后续需要补加机器人控制以及识别相关内容）
        ...
        for attempt in range(max_retries):
            try:
                self.controller = CarController(port='/dev/ttyUSB0')
                self.controller.initialize_car()
                time.sleep(1)
                if self.controller.connect():
                    print(0)
                pos1, rot1 = self.controller.get_agent_state()
                success = self.controller.move(0.01)
                self.queue = []
                print("Connected!")
                break
    ...
    def get_image_information(self):
        ...
        objects=[]
        obs_rgb,obs_dep=self.controller.get_images()
        results = self.model(obs_rgb[:,:,:3], show=False)
        depth=obs_dep
        r = results[0]
        object = []
        for box in r.boxes:
            ...
            drone_pose,drone_rot=self.controller.get_agent_state()
    ...
    def update_graph_and_semantics(self):
        ...
        try:
            ...
            vehicle_pose,vehicle_rot = self.controller.get_agent_state()
            current_position = vehicle_pose
            ...
            if should_add_node:
                position,yaw=self.controller.get_agent_state()
...
if __name__ == "__main__":
    ...
    explorer = None
    try:
        air=AirSimExplorer("/home/wheeltec/test_file/config/config.json")
        time_start=time.time()
        while True:
            time_end=time.time()
