...
from airsim_explorer_1 import AirSimExplorer
import uvicorn
import asyncio
...
# air.habitat.agent.set_state(state)
    ...
    try:
        ...
        while True:
            try:
                ...
                if key == "w":
                    air.habitat.sim.step("move_forward")
                elif key == "a":
                    air.habitat.sim.step("turn_left")
                elif key == "d":
                    air.habitat.sim.step("turn_right")
            except asyncio.TimeoutError:
                pass  # 没有按键，继续循环

            # 获取图像
            obs = air.habitat.sim.get_sensor_observations()
            ...
            results = air.model(rgb_bgr[:,:,:3], show=False)
            r1 = results[0]
            r=r1.plot()
